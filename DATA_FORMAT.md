...
GM-authoritative tactical tracker for RW 4.1's "Seekampf" chapter (p.3-5, `hausregeln/quellen/rw41.txt` line ~109-245) - `seekampf.js`, only active while `eldaraAktiv()`. The GM's own state lives at `localStorage["htbah_gm_seekampf"]` (combat state: units, wind, initiative, log) and `localStorage["htbah_gm_seekampf_karte"]` (map state: token positions, zoom-independent grid coordinates, optional background image), kept as two separate keys since the map state can carry a large embedded image. Unlike the fully GM-local NSC-Liste, this feature *does* reach players (unfiltered - full transparency, matching how the group would see a physical battle map at the table): every mutation is broadcast live, debounced.

The map itself is `battlemap.js`, copied unmodified from the sister project at `E:\vibe_coding\demonslayer\battlemap.js` (a Canvas2D-rendered grid/token/pan-zoom module built for Dungeonslayers, deliberately rule-system-agnostic - see its own header comment). `seekampf.js` calls `BattleMap.create(canvas, { einheit: 200, einheitName: 'm', onChange })` (1 Feld = 200m per the rulebook's Spielfeld section) and keeps ship tokens (`skMap.addFigur`) in sync with its own unit list by `id`. Re-running `BattleMap.create()` on every render would drop the module's pan/zoom/closure state, so `renderSeekampfGm()` builds the `<canvas>` and its toolbar **once** (guarded by `!document.getElementById('sk-canvas')`) and only replaces an inner `#sk-dynamic` container on subsequent renders.
  ...
  "einheiten": [
    {
      ...
      "klasse": "schoner", "spielerIds": ["peer-abc123", "peer-def456"],
...
Both markers and ships get an icon "portrait" via `skMap.setFigurBild()` instead of the plain color-plus-initials `battlemap.js` falls back to otherwise: `skIconBild(icon, farbe)` renders one emoji onto a small offscreen-canvas circle in the given color and returns a cached data URL (cache key `icon+farbe`, so e.g. all Spieler-side ships share one cached image). Ships all use the same sailboat emoji (`SK_SCHIFF_ICON`, colored by `seite`) - `battlemap.js` has no per-class icon concept, and the name label under the token plus the side color already distinguish individual ships. Portraits aren't part of the map's own persisted state (by that module's own design - see its header comment), so `skFigurenAbgleichen()` re-applies every unit's and marker's icon on every sync rather than only at creation, which is cheap since `setFigurBild` already no-ops on an unchanged data URL. **This applies on both sides of the wire**: the GM's `skFigurenAbgleichen()` re-applies icons to `skMap`, and `skEmpfangen()` does the same for `skSpielerMap` right after `applyState()` (portraits travel over neither `getState()` nor the `seekampf` message, so each side must render them locally from the same `farbe`/`typ` data it already has) - the player-side call was missing in an earlier pass, which silently left every player-visible token as a plain colored circle with no icon even though the GM's own view looked correct. `battlemap.js` also exposes freehand/line/circle/rectangle drawing (`setWerkzeug('malen')`, `setMalArt`, `setMalFarbe`) and those shapes (`formen` in the map's own state, not in `seekampf`) for the same purpose - marking a reef zone, cannon range, or fog bank without a discrete token. Both markers and drawn shapes survive `skGefechtZuruecksetzen()` (which only clears `einheiten`/`initiative`/`log` and the matching map figures) since they represent map geography that outlives any one encounter, not combat participants.

**Grid appearance** (`renderSkRasterWerkzeuge()`) exposes `battlemap.js`'s `raster` settings so the overlay can be matched to an uploaded map image, mirroring the same controls in the sister project's `mapui.js`: `rasterSichtbar`/`einrasten` toggles, a `SK_RASTER_FARBEN` preset color swatch row (only shown while the grid is visible) driving `rasterFarbe`, and three number inputs (`rasterGroesse` in pixels per cell, `rasterVersatzX`/`rasterVersatzY` to shift the grid) all calling `skMap.setRaster({...})`. Since `setRaster()` itself calls the map's `melden()` → `onChange`, a grid adjustment is persisted and broadcast to players exactly like a ship move - they always see the same grid the GM tuned in, not just the same tokens.

Each of those three number fields is wrapped in a `.sk-num-stepper` (`−`/`+` buttons either side) rather than left as a bare `<input type="number">` - also ported from `mapui.js`/`app.js` in the sister project, generic and delegated (`skNumStepperTick` plus global `click`/`pointerdown` listeners in seekampf.js, not wired per-instance) so any future `.sk-num-stepper` element works automatically. A click steps by the input's `step` attribute (Feldgröße uses `step="5"` given its 4-400 range; the Versatz fields use the default of 1) respecting `min`/`max`; holding a button auto-repeats after a 350ms delay at 70ms intervals, matching the reference's feel exactly.
...
| Direction | Message | Meaning |
|---|---|---|
| GM -> players | `{ "type": "seekampf", "wind", "runde", "einheiten": [...], "marker": [...], "karte": {...battlemap state...} }` | Complete, unfiltered state - sent after every change (debounced 300ms via `skVerteilen()`/`skJetztVerteilen()`) and on join (`skAnVerbindung`) |
| player -> GM | `{ "type": "seekampfZug", "einheitId", "x", "y" }` | Proposed new position for a ship the sending player owns |

`einheiten[].spielerIds` (array of peerIds, possibly empty) are the GM-assigned crew - toggled via per-player name chips on each unit card (only shown once players are connected; click adds/removes that peerId via `skEinheitSpielerToggle`), so a shared crew ship (the normal Eldara case - the whole party usually shares one ship, not one each) can have several owners at once. `battlemap.js`'s own `besitzer`/`setBesitzer()` mechanism only supports a single value compared by strict equality, and is deliberately left unmodified - so ownership is resolved **client-side, per player**: each player has their own independent `BattleMap` instance (`skSpielerMap`), and after every `applyState()`, `skEmpfangen()` computes locally whether `meinPeer` is present in that unit's `spielerIds` and calls `skSpielerMap.addFigur({id, besitzer: gehoertMir ? meinPeer : null})` - so a ship shared by three players ends up with `besitzer` set to *different* values across three different browsers, each matching only that browser's own `setBesitzer(meinPeer)` call. The GM's own `skMap` never sets `besitzer` at all (irrelevant there - the GM's map has no `nurEigene` restriction and can drag anything). A player's own instance is created with `bestaetigungNoetig: true` (a drag sets `geplantX/geplantY` locally and fires `onZugVorschlag` instead of moving immediately), so only an owned ship is draggable, and even that only produces a proposal. The proposal is sent as `seekampfZug`; `skAnfrageVerarbeiten` (GM side) re-checks membership server-side (`!e.spielerIds.includes(peerId)` is rejected silently) before merging `geplantX/geplantY` onto the GM's own copy of that figure via `skMap.addFigur()` - which reuses `battlemap.js`'s own pending-move rendering (the dashed preview) and its `offeneZuege()`/`zugBestaetigen()`/`zugVerwerfen()` API unmodified, just fed by network messages instead of local drags. The GM sees pending proposals in a dedicated panel with Bestätigen/Verwerfen buttons; confirming actually moves the ship (and re-broadcasts), rejecting just clears the proposal. Older saved state from before this used a single `spielerId` - `skLaden()` migrates it into a one-element `spielerIds` array and drops the old field.

Players get a separate, read-only-by-default render (`renderSeekampfSpieler()`, `#seekampf-section` in index.html) with its own `BattleMap` instance (`skSpielerMap`) and a trimmed toolbar (Zeigen/Messen only - drawing, markers and the background image stay GM-only prep tools) plus a read-only unit list (`skEinheitKarteSpielerHtml`, no inputs, no delete, no effect-removal). On first receipt of a `seekampf` message, `skEmpfangen()` must render the shell (creating `skSpielerMap`) *before* calling `applyState()` on it - reversing that order silently drops the very first sync, since `applyState` can't run against a map instance that doesn't exist yet.
