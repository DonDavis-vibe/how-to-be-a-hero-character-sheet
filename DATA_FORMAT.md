...
**Per-map raster (grid) settings**: each map's `zustand.raster` (Feldgröße/Versatz X/Versatz Y/Farbe/sichtbar/einrasten) is meant to be fully independent per map, but `battlemap.js`'s own `applyState()` *merges* the incoming raster onto the live instance's current one (`if (neu.raster) Object.assign(zustand.raster, neu.raster)`) rather than replacing it - a partial or empty raster object on the incoming map would silently keep whatever the previously active map had, since `Object.assign` only overwrites the keys actually present. Since `battlemap.js` itself is never edited, `karten.js` works around this by never handing it a partial object: `KARTE_RASTER_STANDARD` (a literal copy of `battlemap.js`'s own internal `STANDARD` raster defaults) is what `karteLeererZustand()` uses for brand-new maps, and `karteLaden()` backfills any saved map's `zustand.raster` against it (`Object.assign({}, KARTE_RASTER_STANDARD, gespeichertesRaster)`, own values winning) on every load - both paths guarantee every raster field is always present, so the `Object.assign` merge inside `applyState()` ends up behaving like a full replace in practice. Without this, a brand-new map would start at whatever Feldgröße the GM had last dialed in on a different map instead of the 50px/field default, and older saved maps (from before this fix) would have had an empty `raster: {}` that behaved the same way.
